...
describe('correções pré-implantação 0.7.1', () => {
  ...
  it('região, baseline Node e Worker são preservados na linha ativa 1.0.0', () => {
    ...
    expect(rootPackage).toMatchObject({ version: '1.0.0', engines: { node: '>=22.22.2 <23' } });
    expect(workerPackage).toMatchObject({ version: '1.0.0', engines: { node: '>=22.22.2 <23' } });
    expect(workerIndex).toMatch(/version:\s*['"]1\.0\.0['"]/);
