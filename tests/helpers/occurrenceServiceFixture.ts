...
import { DEFAULT_OPERATIONAL_CONFIG, REFERENCE_LOCATIONS, REFERENCE_CATEGORIES, INITIAL_INTAKE_TEAM } from '../../server/repositories/referenceSeedData';
...
export function makeOccurrenceServiceFixture(){
 ...
 const attendant=profile('Atendente','atendente@ifes.edu.br',['team-admin']);const secondAttendant=profile('Atendente','atendente2@ifes.edu.br',[]);
 const adminUsers=new InMemoryAdminUserRepository([adminUser(administrator),adminUser(manager),adminUser(secondManager),adminUser(attendant),adminUser(secondAttendant)]);const auditLogs=new InMemoryAuditLogRepository();
 const initialTeam:OperationalTeam={...INITIAL_INTAKE_TEAM,memberAdminUserIds:[administrator.id,manager.id,secondManager.id]};
 const team:OperationalTeam={id:'team-admin',schemaVersion:2,name:'Coordenação Geral de Administração / Engenharia',active:true,sortOrder:1,isInitialIntakeTeam:false,memberAdminUserIds:[administrator.id,manager.id,attendant.id],createdAt:'2026-08-10T00:00:00.000Z',createdBy:'test',updatedAt:'2026-08-10T00:00:00.000Z',updatedBy:'test'};
 const teams=new FakeOperationalTeamRepository([initialTeam,team]);const sla=new FakeSlaConfigRepository();
 ...
 return{service,occurrences,events,adminUsers,auditLogs,teams,sla,administrator,manager,secondManager,attendant,secondAttendant};
}
