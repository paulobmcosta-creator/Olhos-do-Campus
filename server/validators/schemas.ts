...
import { INTERNAL_NOTE_AUDIENCES, OCCURRENCE_PRIORITIES, OCCURRENCE_STATUSES, OCCURRENCE_STATUS_VALUES } from '../../src/models/occurrence';
...
export const updateOccurrenceSchema=z.object({
 ...
 assignedTeamId:id.nullable().optional(),assignedToAdminUserId:id.nullable().optional(),newPublicMessage:z.string().trim().min(1).max(1000).optional(),newInternalNote:z.string().trim().min(1).max(1000).optional(),internalNoteAudience:z.enum(INTERNAL_NOTE_AUDIENCES).optional(),
 dataClassification:z.enum(['REAL','TEST']).optional(),attachmentTargetProtocol:z.string().trim().regex(/^[A-Z0-9]+(?:-[A-Z0-9]+)*-\d{4}-\d{6}$/iu).nullable().optional(),attachmentRelation:z.enum(['DUPLICATE','SIMILAR']).optional(),attachmentReason:z.string().trim().min(10).max(500).optional(),applyPublicMessageToAttached:z.boolean().optional(),
}).strict().superRefine((v,ctx)=>{if(!Object.entries(v).some(([k,x])=>k!=='expectedVersion'&&x!==undefined))ctx.addIssue({code:'custom',message:'Informe ao menos uma alteração.'});if(v.categoryId&& !v.categoryChangeReason)ctx.addIssue({code:'custom',path:['categoryChangeReason'],message:'A justificativa da recategorização é obrigatória.'});if(v.location&&!v.locationChangeReason)ctx.addIssue({code:'custom',path:['locationChangeReason'],message:'A justificativa da correção de local é obrigatória.'});if(v.newInternalNote===undefined&&v.internalNoteAudience!==undefined)ctx.addIssue({code:'custom',path:['internalNoteAudience'],message:'A audiência somente pode ser informada com uma observação interna.'});if(v.attachmentTargetProtocol!==undefined&&!v.attachmentReason)ctx.addIssue({code:'custom',path:['attachmentReason'],message:'A justificativa do apensamento ou desapensamento é obrigatória.'});if(typeof v.attachmentTargetProtocol==='string'&&!v.attachmentRelation)ctx.addIssue({code:'custom',path:['attachmentRelation'],message:'Informe se a ocorrência é duplicada ou similar.'});if(v.attachmentTargetProtocol===undefined&&v.attachmentRelation!==undefined)ctx.addIssue({code:'custom',path:['attachmentRelation'],message:'A relação somente pode ser informada ao apensar uma ocorrência.'});if(v.applyPublicMessageToAttached===true&&v.newPublicMessage===undefined)ctx.addIssue({code:'custom',path:['applyPublicMessageToAttached'],message:'A propagação exige uma nova mensagem pública.'});});
