...
export const configUpdateSchema=z.object({institutionDisplayName:z.string().trim().min(3).max(160).optional(),protocolPrefix:z.string().trim().min(1).max(20).optional(),notificationEmails:z.array(z.string().email()).max(20).optional(),autoAssignRisk:z.boolean().optional(),serviceNotice:z.string().trim().max(500).optional()}).strict().refine(v=>Object.values(v).some(x=>x!==undefined),{message:'Informe ao menos uma configuração para atualização.'});
...
export const legacyAdminResolutionSchema=z.object({action:z.enum(['CONVERT_TO_MANAGER','DEACTIVATE'])}).strict();
...
export const teamCreateSchema=z.object({name:z.string().trim().min(2).max(160),description:z.string().trim().max(500).optional(),active:z.boolean().optional(),sortOrder:z.number().int().min(0).max(10000),memberAdminUserIds:z.array(id).max(100).optional()}).strict();
export const teamUpdateSchema=z.object({name:z.string().trim().min(2).max(160).optional(),description:z.string().trim().max(500).optional(),active:z.boolean().optional(),sortOrder:z.number().int().min(0).max(10000).optional(),memberAdminUserIds:z.array(id).max(100).optional()}).strict().refine(v=>Object.values(v).some(x=>x!==undefined),{message:'Informe ao menos uma alteração.'});
