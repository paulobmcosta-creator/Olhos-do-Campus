 ...
 public async update(id:string,input:UpdateOccurrenceInput,author:AuthorizedAdminProfile,correlationId:string):Promise<Occurrence>{
   ...
   if(input.attachmentTargetProtocol!==undefined){
     ...
     if(input.attachmentTargetProtocol===null){
       ...
       if(previousOperational.assignedTeamId!==next.assignedTeamId)eventList.push(adminEvent(next.assignedTeamId?'TEAM_CHANGED':'TEAM_CHANGED','INTERNAL',now,author,correlationId,{internalDescription:'Equipe responsável sincronizada em razão do apensamento.',previousValue:previousOperational.assignedTeamId,newValue:next.assignedTeamId}));
